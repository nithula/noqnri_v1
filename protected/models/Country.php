...
class Country extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('country_code, country_name, country_phone_code', 'required'),
			array('country_phone_code', 'numerical', 'integerOnly'=>true),
			array('country_code', 'length', 'max'=>3),
			array('country_name', 'length', 'max'=>150),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, country_code, country_name, country_phone_code', 'safe', 'on'=>'search'),
		);
	}
